...
const checks = [
  ...
  { name: 'japanese_past_exam_style_present', pass: scenarioText.includes('japanese-scenario') && scenarioText.includes('譛繧る←蛻・) && scenarioText.includes('隱､縺｣縺ｦ縺・ｋ') && scenarioText.includes('莉･荳・), value: files.scenarios },
  { name: 'long_form_trace_present', pass: scenarioText.includes('long-trace') && scenarioText.includes('traceSteps') && scenarioText.includes('莠悟・謗｢邏｢縺ｮ螳悟・繝医Ξ繝ｼ繧ｹ'), value: files.scenarios },
  { name: 'sql_step_by_step_present', pass: scenarioText.includes('sql-step') && scenarioText.includes('sqlSteps') && scenarioText.includes('3陦ｨJOIN縺ｨ髮・ｨ・), value: files.scenarios },
  { name: 'scenario_drill_component_renders_traps', pass: fs.existsSync(files.scenarioComponent) && read('scenarioComponent').includes('譌･譛ｬ隱・trap / exam tip') && read('scenarioComponent').includes('TraceTable') && read('scenarioComponent').includes('SqlStepTable'), value: files.scenarioComponent },
  { name: 'practice_embeds_scenario_drills', pass: read('practiceDrills').includes('FundamentalInfoScenarioDrills') && (read('practiceDrills').includes('V61 遘醍岼B Drill') || read('practiceDrills').includes('V62 遘醍岼B Drill') || read('practiceDrills').includes('V63 遘醍岼B Drill')), value: files.practiceDrills },
  ...
  { name: 'focus_mode_points_to_v61_scenarios', pass: (read('focus').includes('V61 Fundamental Info Focus') || read('focus').includes('V62 Fundamental Info Focus') || read('focus').includes('V63 Fundamental Info Focus')) && (read('focus').includes('V61 Scenario Drill') || read('focus').includes('V62 Scenario Drill') || read('focus').includes('V63 遘醍岼B Deep Drill')), value: files.focus },
