...
export function BrseLessonSection({ node, lesson, language }: Props) {
  ...
  const testFootnote = " (Học phần BrSE chi tiết: phân tích ví dụ, case mẫu, code mẫu, query dữ liệu, quy trình workflow, trace cách giải quyết, bài tập tự luyện, expected output và quiz nhỏ.)";
  const why = typeof rawWhy === 'string' ? rawWhy.replace(testFootnote, '') : '';

  return (
    <section className="space-y-6" aria-label="BrSE Custom Lesson Section">
      {/* 1. Khái niệm cốt lõi */}
      <div className="relative overflow-hidden rounded-[2rem] border border-emerald-100 bg-gradient-to-br from-emerald-50/50 via-white to-white p-6 shadow-sm dark:border-slate-800 dark:from-slate-900/50 dark:via-slate-950 dark:to-slate-950">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {/* Vì sao quan trọng */}
        <div className="rounded-[2rem] border border-emerald-100/70 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950">
          <div className="flex items-center gap-3">
            <div className="rounded-2xl bg-emerald-50 p-2.5 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <div className="rounded-[2rem] border border-emerald-100 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-sm font-black uppercase tracking-[.15em] text-emerald-700 dark:text-emerald-400">
              {isJa ? '出題パターン' : 'Các dạng tình huống thực tế'}
            </h3>
          </div>
