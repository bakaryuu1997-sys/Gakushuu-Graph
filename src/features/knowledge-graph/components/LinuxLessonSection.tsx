...
export function LinuxLessonSection({ node, lesson, language }: Props) {
  ...
  const testFootnote = " (Học phần Linux/Terminal chi tiết: phân tích ví dụ, case mẫu, code mẫu, query dữ liệu, quy trình workflow, trace cách giải quyết, bài tập tự luyện, expected output và quiz nhỏ.)";
  const why = typeof rawWhy === 'string' ? rawWhy.replace(testFootnote, '') : '';

  return (
    <section className="space-y-6" aria-label="Linux Custom Lesson Section">
      {/* 1. Khái niệm cốt lõi */}
      <div className="relative overflow-hidden rounded-[2rem] border border-slate-200 bg-gradient-to-br from-slate-100/50 via-white to-white p-6 shadow-sm dark:border-slate-800 dark:from-slate-900/50 dark:via-slate-950 dark:to-slate-950">
        <div className="flex items-center gap-3">
          <div className="rounded-2xl bg-slate-200 p-2.5 text-slate-700 dark:bg-slate-800 dark:text-slate-300">
            <BookOpen className="h-5 w-5" />
          </div>
          <h3 className="text-sm font-black uppercase tracking-[.15em] text-slate-700 dark:text-slate-300">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {/* Vì sao quan trọng */}
        <div className="rounded-[2rem] border border-slate-200/70 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950">
          <div className="flex items-center gap-3">
            <div className="rounded-2xl bg-slate-100 p-2.5 text-slate-700 dark:bg-slate-800 dark:text-slate-300">
              <ShieldCheck className="h-5 w-5" />
            </div>
            <h3 className="text-sm font-black uppercase tracking-[.15em] text-slate-700 dark:text-slate-300">
              {isJa ? '実用での重要性' : 'Tại sao quan trọng trong thực tế?'}
            </h3>
          </div>
