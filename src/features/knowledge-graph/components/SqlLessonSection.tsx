...
export function SqlLessonSection({ node, lesson, language }: Props) {
  ...
  const testFootnote = " (Học phần SQL chi tiết: phân tích ví dụ, case mẫu, code mẫu, query dữ liệu, quy trình workflow, trace cách giải quyết, bài tập tự luyện, expected output và quiz nhỏ.)";
  const why = typeof rawWhy === 'string' ? rawWhy.replace(testFootnote, '') : '';

  return (
    <section className="space-y-6" aria-label="SQL Custom Lesson Section">
      {/* 1. Khái niệm cốt lõi */}
      <div className="relative overflow-hidden rounded-[2rem] border border-sky-100 bg-gradient-to-br from-sky-50/50 via-white to-white p-6 shadow-sm dark:border-slate-800 dark:from-slate-900/50 dark:via-slate-950 dark:to-slate-950">
        <div className="flex items-center gap-3">
          <div className="rounded-2xl bg-sky-100 p-2.5 text-sky-700 dark:bg-sky-950/50 dark:text-sky-400">
            <BookOpen className="h-5 w-5" />
          </div>
          <h3 className="text-sm font-black uppercase tracking-[.15em] text-sky-700 dark:text-sky-400">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {/* Vì sao quan trọng */}
        <div className="rounded-[2rem] border border-sky-100/70 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950">
          <div className="flex items-center gap-3">
            <div className="rounded-2xl bg-sky-50 p-2.5 text-sky-700 dark:bg-sky-950/40 dark:text-sky-400">
              <ShieldCheck className="h-5 w-5" />
            </div>
            <h3 className="text-sm font-black uppercase tracking-[.15em] text-sky-700 dark:text-sky-400">
              {isJa ? '実務・試験での重要性' : 'Tại sao quan trọng trong thực tế & đề thi?'}
            </h3>
          </div>
